...
func (j *WorkloadSpec) UnmarshalJSON(b []byte) error {
	...
	if v, ok := raw["apiVersion"]; !ok || v == nil {
		return fmt.Errorf("field apiVersion in WorkloadSpec: required")
	}
	if v, ok := raw["containers"]; !ok || v == nil {
		return fmt.Errorf("field containers in WorkloadSpec: required")
	}
	if v, ok := raw["metadata"]; !ok || v == nil {
		return fmt.Errorf("field metadata in WorkloadSpec: required")
	}
	type Plain WorkloadSpec
	var plain Plain
	if err := json.Unmarshal(b, &plain); err != nil {
		return err
	}
	*j = WorkloadSpec(plain)
	return nil
}
